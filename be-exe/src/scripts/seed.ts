...
//   // ── Timeline ─────────────────────────────────────────────────────────────────
//   console.log("\n🌱 Seeding timeline...");
//   const timelines: { _id: unknown; slug: string; title: string }[] = [];
//   for (const item of timelineData) {
//     const doc = await Timeline.findOneAndUpdate({ slug: item.slug }, item, {
//       upsert: true,
//       new: true,
//     });
//     timelines.push({ _id: doc!._id, slug: item.slug, title: item.title });
//     console.log(`  ✅ ${item.title}`);
//   }

//   // Gán timelineId cho chapter cũ chưa có liên kết
//   const firstTimelineId = timelines[0]?._id;
//   if (firstTimelineId) {
//     await Chapter.updateMany(
//       { $or: [{ timelineId: null }, { timelineId: { $exists: false } }] },
//       { $set: { timelineId: firstTimelineId } },
//     );
//   }

//   // ── Chapters + Lessons theo từng giai đoạn ─────────────────────────────────
//   console.log("\n🌱 Seeding chapters & lessons...");
//   for (let i = 0; i < timelines.length; i++) {
//     const tl = timelines[i];
//     const existing = await Chapter.countDocuments({ timelineId: tl._id });
//     if (existing > 0) continue;

//     const chapter = await Chapter.create({
//       slug: `${tl.slug}-chuong-1`,
//       title: `Khóa học: ${tl.title}`,
//       description: `Nội dung khóa học thuộc giai đoạn ${tl.title}`,
//       order: 1,
//       requiredLevel: i < 2 ? 1 : 2,
//       isPublished: true,
//       timelineId: tl._id,
//     });
...
//         title: "Bài 1: Tổng quan giai đoạn",
//         order: 1,
//         isFree: true,
//         isPublished: true,
//         video: { url: "", provider: "youtube", durationSec: 0 },
//       },
//       {
//         chapterId: chapter._id,
//         title: "Bài 2: Sự kiện tiêu biểu",
//         order: 2,
//         isFree: i === 0,
//         isPublished: true,
//         video: { url: "", provider: "youtube", durationSec: 0 },
//       },
//     ]);
//     console.log(`  ✅ Chapter + lessons cho: ${tl.title}`);
//   }
